...
{
	int out_dec_num{};
	map <char, int> DecimalValueTranslator;

	DecimalValueTranslator['M'] = 1000;
	DecimalValueTranslator['D'] = 500;
	DecimalValueTranslator['C'] = 100;
	DecimalValueTranslator['L'] = 50;
	DecimalValueTranslator['X'] = 10;
	DecimalValueTranslator['V'] = 5;
	DecimalValueTranslator['I'] = 1;
	
	size_t len = Roman_str.length();

	for (int i = 0; i < len; i++) {
		int w1 = DecimalValueTranslator[Roman_str[i]];
		if (i + 1 < len) {
			int w2 = DecimalValueTranslator[Roman_str[i+1]];
			if (w1 >= w2) {
				out_dec_num += w1;
			}
			else {
